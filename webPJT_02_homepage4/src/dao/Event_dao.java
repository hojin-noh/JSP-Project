...
public class Event_dao {
	...
	public int updateEvent(Event_dto dto){
		...
		try {
			...
			System.out.println(" updateEvent() query 오류 " + query);
		}catch(Exception e) {
			System.out.println(" updateEvent() 오류 ");
		}finally {
			common.close(connection, ps, rs);
		}
	...
	public int SaveEvent(Event_dto dto){
		...
		String query = "insert into h02_"+pageType+"\r\n" + 
					...
					+ "'"+dto.getS_date()+"','"+dto.getE_date()+"','"+dto.getReg_name()+"','"+dto.getReg_date()+"')";
		...
		try {
			...
			System.out.println(" SaveEvent() query 오류 " + query);
		}catch(Exception e) {
			System.out.println(" SaveEvent() 오류 ");
		}finally {
			common.close(connection, ps);
		}
