	...
	<div class="board_list">
		<table class="board_table">
			...
			<%for(int k=0; k < arr.size(); k++){ %>
				<tr>
					<td><a href="event_view.jsp?t_event_no=<%=arr.get(k).getNo() %>"><%=arr.get(k).getNo() %></a></td>					
					<!-- <td><a href="javascript:goView('<%=arr.get(k).getNo() %>')"><%=arr.get(k).getNo() %></a></td>	post방식으로 넘기는 방법(번호 클릭시) -->				
					<td class="title"><a href="javascript:goView('<%=arr.get(k).getNo() %>')"><%=arr.get(k).getTitle() %></td>					
